...
test X fail, compile my VUnit project, list VUnit tests, which files do I need
...
| `vunit_list_tests` | All test cases as `lib.entity[.proc]`. Use to get exact names and to check patterns before a run. | no |
...
| `vunit_get_report` | Re-read the **last run's** JUnit report (no re-run). Per-test status + failing-check counts per test. | no |
| `vunit_get_test_log` | The per-test log (last 100 lines by default — failure info is at the end). The way to see **why** a test failed; failing checks get a structured summary. | no |
...
- `test_patterns` — VUnit patterns like `lib.entity[.proc]`; default `['*']` runs everything.
...
**"Why did test X fail?"**
→ `vunit_get_test_log(test_name="lib.entity.proc")`. If you don't have the
...
## Rules of thumb
- Test names are always the full `lib.entity[.proc]` form from
